 ...
 *------------------------------------------------------------------------
 */
SOCKET sockets[50]={NULL},ssock,msock; 
time_t now;
char *pts;
char buffer[1000];
int cc;
int addr;
unsigned threadid,threadID[50];
...
unsigned int __stdcall CHAT(PVOID PM)
{
	...
	for(int i=0;i<=addr;i++)
	{
		if(sockets[i]!=NULL)
		{
			send(sockets[i],buffer1,sizeof(buffer1),0);
		}
	}
loop:	cc=recv(sock,buffer2,1000,0);
	if(cc==SOCKET_ERROR||cc==0)
	{
		time(&now);
		pts=ctime(&now);
		sprintf(buffer2,"用户[ip:%s 端口:%d]: leave.\n时间:%s\n",inet_ntoa(fsin.sin_addr), fsin.sin_port,pts);
		sock=NULL;
		sockets[addr]=NULL;
		CloseHandle(hThread[addr]);
		printf("%s",buffer2);
		for(int i=0;i<=addr;i++)
		{
			if(sockets[i]!=NULL&&sockets[i]!=sock)
			{
				send(sockets[i],buffer2,sizeof(buffer2),0);
			}
		}
	}
	else
	{
		time(&now);
		pts=ctime(&now);
		sprintf(buffer3,"用户[ip:%s 端口:%d]: %s.\n时间:%s\n",inet_ntoa(fsin.sin_addr), fsin.sin_port,buffer2, pts);
		printf("%s",buffer3);
		for(int i=0;i<=addr;i++)
		{
			if(sockets[i]!=NULL)
			{
				send(sockets[i],buffer3,sizeof(buffer3),0);
			}
		}
		goto loop;
	}
                     ...
                     argc=2 argv[0]="TCPServer",argv[1]="8080" */
{
    ...
    while(1){ 		                                   // 检测是否有按键,如果没有则进入循环体执行
       ...
       hThread[addr]=(HANDLE)_beginthreadex(NULL,0,CHAT,NULL,0,&threadid);
	   addr++;
